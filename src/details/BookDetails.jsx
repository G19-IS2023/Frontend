  ...
  const fetchAndUpdatePagesRead = async () => {
    try {
      const { data: libraries } = await axios.get(
        `${process.env.URL}/book/getLibraries/${userId}`
      );
      let foundBook = false;
      ...
      for (let library of libraries) {
        try {
          const res = await axios.get(
            `${process.env.URL}/book/library/${library.libId}/getBook/${id}/id/${userId}`
          );
  ...
  const addBookToLibraryAndSavePages = async () => {
    try {
      if (!bookAddedToLibrary.current && book.num_pages > 0 && readPages > 0) {
        ...
        await axios.post(`${process.env.URL}/book/addBook`, {
          bookId: id,
          libId: "1",
          userId: userId,
          pages: readPages, // Usa direttamente readPages invece di debouncedReadPages
        });
        await axios.put(`${process.env.URL}/book/modifyPages`, {
          bookId: id,
          libId: "1",
          pages: readPages,
          userId: userId,
